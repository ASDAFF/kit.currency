	...
	for($i=0; $i<count($arAllOptions); $i++)
	{
		...
		if($name == 'CURRENCY_AUTO_UPDATE_PERIOD'){
			...
			if($valOld != $date_start){
				CAgent::RemoveAgent("EMBXCurrencyAuto::updateRates();", $module_id);
				CAgent::AddAgent("EMBXCurrencyAuto::updateRates();", $module_id, "N", 86400, $date_start, "Y", $date_start);
		...
		if($name == 'CURRENCY_AUTO_UCACHE_PERIOD'){
			...
			if($valOld != $date_start){
				CAgent::RemoveAgent("EMBXCurrencyAuto::clearCache();", $module_id);
				CAgent::AddAgent("EMBXCurrencyAuto::clearCache();", $module_id, "N", 86400, $date_start, "Y", $date_start);
